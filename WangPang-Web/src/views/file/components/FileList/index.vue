...
export default {
  ...
  methods: {
    ...
    getTableDataByType() {// 获取文件列表数据
      this.batchOperate = false
      this.loading = true
      if (Number(this.fileType)) {// 分类型进行查看
        if (Number(this.fileType) === 6) {
          this.shwoFileRecovery() // 查看回收站文件列表
        } else {//根据类型文件进行查看
    ...
    getFileList() {// 获取文件平铺数据
      this.pageData.currentPage++
      this.getTableDataByType()
    },
    handleCurrentChange(currentPage) {// 分页组件 当前页码改变
      this.pageData.currentPage = currentPage
      this.getTableDataByType()
    },
    shwoFileRecovery() { // 获取回收站文件列表
    ...
    showFileListByType() {  // 根据文件类型展示文件列表
      let data = {//  分类型
