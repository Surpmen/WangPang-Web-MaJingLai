...
<script>
    ...
    export default {
        data() {
            return {
                options: {
                    ...
                    chunkSize: 1024 * 1024 * 50, //每个分片大小
                    fileParameterName: 'file',//上传文件时文件的参数名，默认file
                    maxChunkRetries: 3,//最大自动失败重试上传次数
                    testChunks: true,   //是否开启服务器分片校验
                    checkChunkUploadedByResponse: function (chunk, message) {// 服务器分片校验函数，秒传及断点续传基础
                        let objMessage = JSON.parse(message);//获取服务器返回的已上传分片信息
                        let data = objMessage.data;
                        if (data.skipUpload) {//秒传
                            return true;
                        }
                        return (data.uploaded || []).indexOf(chunk.offset + 1) >= 0 //上传未上传的分片
                    },
                    headers: {
        ...
        methods: {
            filesAdded(file) { // 添加文件的回调
                this.panelShow = true;
                this.computeMD5(file);
            },
            onFileProgress(rootFile, file, chunk) {
            },
            onFileSuccess(rootFile, file, response, chunk){  // 文件上传成功的回调
                if (response == "") {
            ...
            computeMD5(file) {//计算md5，实现断点续传及秒传
                let fileReader = new FileReader();//初始化FileReader
                let time = new Date().getTime();//获取时间
                let blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice; //当前分片编号
                let currentChunk = 0;
                const chunkSize = 50 * 1024 * 1024;//分片大小
                let chunks = Math.ceil(file.size / chunkSize); //计算分片数
                let spark = new SparkMD5.ArrayBuffer(); //初始化sparkMD5
            ...
            computeMD5Success(md5, file) {// 将自定义参数直接加载uploader实例的opts上
